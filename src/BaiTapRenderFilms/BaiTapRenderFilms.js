...
export default class BaiTapRenderFilms extends Component {
        ...
        let contentFilms = dataFilms.map((film, index) => {
            return <div className="col-3 mt-3" key={index}>
                <div className="card text-white bg-primary ml-5" style={{width: '250px'}}>
                    <img style={{width:'250px', height:'250px'}} className="card-img-top" src={film.hinhAnh} alt={film.tenPhim} />
                    <div className="card-body">
                        <h4 className="card-title" style={{fontSize: '17px', height: '50px'}}>{film.tenPhim}</h4>
                        <p className="card-text" style={{fontSize: '13px', height: '50px'}}>{film.moTa.length>80 
                        ? <p>{film.moTa.substring(0,80)} ...</p> : <p>{film.moTa}</p> }</p>
                    </div>
                </div>
            </div>
        });
    ...
    render() {
        return (
            <div style={{backgroundImage:'url(./background/avengers.jpg)', minHeight:'4000px'}}>
                <div style={{ backgroundColor: 'rgba(0,0,0,0.5)', minHeight:'4000px'}}>
